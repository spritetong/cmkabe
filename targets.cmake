...
# Check the Zig toolchain.
if ("${TARGET_C_COMPILER}" MATCHES "zig-[^-]*${CMKABE_EXE_EXT}$")
    set(ZIG ON)
else()
    set(ZIG OFF)
endif()
...
if(NOT DEFINED _CMAKABE_TARGET_INITIALIZED)
    # Set the native triple
    if("${TARGET}" MATCHES "^(|native)$")
        set(TARGET "${HOST_TRIPLE}")
        set(TARGET_TRIPLE "${HOST_TRIPLE}")
    elseif("{TARGET_TRIPLE}" STREQUAL "")
    ...
    elseif(("${TARGET}" STREQUAL "${HOST_TRIPLE}") AND NOT ZIG)
        set(_target_type "native")
    elseif("${TARGET}" MATCHES "android")
        ...
        elseif("${TARGET_TRIPLE}" MATCHES "^([^-]+)(.*)$")
            # Check if `system-vendor-env` of ${TARGET_TRIPLE} matches that of ${HOST_TRIPLE}.
    ...
    elseif(_target_type STREQUAL "")
        message(FATAL_ERROR "Unrecognized target triple: ${TARGET_TRIPLE}")
    endif()
    ...
    elseif(_target_type STREQUAL "android")
        if(NOT "${TARGET_TRIPLE}" MATCHES "^([^-]+)-([^-]+)-(android|androideabi)$")
    ...
    elseif(_target_type STREQUAL "gcc")
        ...
        if(ZIG)
            set(CMAKE_C_COMPILER "${_prefix}cc${CMKABE_EXE_EXT}" CACHE INTERNAL "C compiler" FORCE)
            set(CMAKE_CXX_COMPILER "${_prefix}c++${CMKABE_EXE_EXT}" CACHE INTERNAL "CXX compiler" FORCE)
            set(CMAKE_ASM_COMPILER "${_prefix}cc${CMKABE_EXE_EXT}" CACHE INTERNAL "ASM compiler" FORCE)
            set(CMAKE_ASM-ATT_COMPILER "${_prefix}cc${CMKABE_EXE_EXT}" CACHE INTERNAL "ASM-ATT compiler" FORCE)
            set(CMAKE_RC_COMPILER "${_prefix}rc${CMKABE_EXE_EXT}" CACHE INTERNAL "RC compiler" FORCE)
            set(CMAKE_AR "${_prefix}ar${CMKABE_EXE_EXT}" CACHE INTERNAL "LLVM archiver" FORCE)
            set(CMAKE_RANLIB "${_prefix}ranlib${CMKABE_EXE_EXT}" CACHE INTERNAL "LLVM ranlib" FORCE)
        else()
            set(CMAKE_C_COMPILER "${_prefix}gcc${CMKABE_EXE_EXT}" CACHE STRING "C compiler" FORCE)
            set(CMAKE_CXX_COMPILER "${_prefix}g++${CMKABE_EXE_EXT}" CACHE STRING "CXX compiler" FORCE)
            set(CMAKE_ASM_COMPILER "${_prefix}gcc${CMKABE_EXE_EXT}" CACHE STRING "ASM compiler" FORCE)
            set(CMAKE_ASM-ATT_COMPILER "${_prefix}as${CMKABE_EXE_EXT}" CACHE STRING "ASM-ATT compiler" FORCE)
            set(CMAKE_AR "${_prefix}ar${CMKABE_EXE_EXT}" CACHE INTERNAL "GNU archiver" FORCE)
            set(CMAKE_RANLIB "${_prefix}ranlib${CMKABE_EXE_EXT}" CACHE INTERNAL "GNU ranlib" FORCE)
        endif()
