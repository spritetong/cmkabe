...
class TargetParser:
    ...
    def __init__(
        ...
        **_args: Any,
        ...
        self.cmake_target_prefix: str = normpath(
            os.path.abspath(
                cmake_target_prefix
                or (self.workspace_dir + "/installed")
            )
        )
    ...
    def target_is_runnable(self) -> bool:
        if (
            self.target_is_native
            or self.cargo_target == self.host_cargo_target
        ):
            return True
        if (
            not self.android
    ...
    def enum_prefix_subdirs_of(
        ...
        if subdir in ["bin", "lib"]:
            if make:
                for directory in [self.cargo_out_dir(make=True), self.cmake_build_dir()]:
                    res.append('"{}"'.format(directory) if quotes else directory)
            elif cmake:
                for directory in [self.cargo_out_dir(cmake=True), "${CMAKE_BINARY_DIR}"]:
                    res.append('"{}"'.format(directory) if quotes else directory)
    ...
    def parse(self) -> "TargetParser":
        ...
        self.zig = (
            os.path.splitext(os.path.basename(self.target_cc))[0]
            in ("zig", "zig-cc")
        )
        ...
        if (
            ...
            target_cc = shutil.which(
                self.target + "-gcc" + EXE_EXT
            ) or shutil.which(self.target + "-cc" + EXE_EXT)
            if target_cc:
        ...
        self.cmake_prefix_subdirs = [
            "{}/{}".format(self.cmake_target_prefix, x)
    ...
    def _zig_init(self) -> None:
        ...
        self.target_cc = normpath(
            self.zig_cc_dir + "/zig-cc" + EXE_EXT
        )
    ...
    def _android_init(self) -> None:
        ...
        if not self.android_ndk_root or not os.path.isdir(
    ...
    def _build(self) -> None:
        ...
        def join_paths(paths: List[str], subdirs: Optional[List[str]] = None) -> List[str]:
            return (
                ["/".join([p, s]) for p in paths for s in subdirs]
                if subdirs
                else paths
            )

        def make_export_paths(name: str, paths: List[str], subdirs: Optional[List[str]] = None) -> str:
            value = (
                os.pathsep
                + os.pathsep.join(join_paths(paths, subdirs))
                + os.pathsep
            )
            export_only = False
            if name == "PATH":
        ...
        def cmake_export_paths(name: str, paths: List[str], subdirs: Optional[List[str]] = None) -> str:
        ...
        with open(
            os.path.join(self.cmake_target_dir, ".settings.mk"), "wb"
        ) as f:
        ...
        with open(
            os.path.join(self.cmake_target_dir, ".settings.cmake"), "wb"
        ) as f:
            fwrite(f, "# Home directory\n")
            ...
            fwrite(
                ...
                "set(TARGET_PREFIX_INCLUDES {})\n".format(
                    " ".join(
                        self.enum_prefix_subdirs_of(
                            "include", quotes=True, cmake=True
                        )
                    )
                ),
            )
        ...
        if self.android:
            ...
            if target_cc_abs.endswith("-gcc"):
                ...
                raise ValueError(
                    "Unrecognized target CC: {}".format(self.target_cc)
                )
        ...
        with open(
            os.path.join(self.cmake_target_dir, ".environ.mk"), "wb"
        ) as f:
            if cc_exports:
            ...
            cargo_target = (
                "CARGO_TARGET_" + self.cargo_target.upper().replace("-", "_")
            )
            if cc:
                fwrite(f, "# LINKER\n")
        ...
        with open(
            os.path.join(self.cmake_target_dir, ".environ.cmake"), "wb"
        ) as f:
            if cc_exports:
                for line in cc_exports:
                    k, v = list(map(lambda x: x.strip(), line.split("=", 1)))
            ...
            fwrite(
                f, 'set(ENV{{CMKABE_TARGET}} "{}")\n'.format(self.cmkabe_target)
            )
            fwrite(
                f, 'set(ENV{{CMKABE_TARGET_DIR}} "{}")\n'.format(self.target_dir)
            )
            fwrite(
                f,
            ...
            fwrite(
                f, 'elseif(CMAKE_BUILD_TYPE_LOWER STREQUAL "minsizerel")\n'
            )
            fwrite(f, "    set(ENV{CMKABE_DEBUG} OFF)\n")
            fwrite(f, "    set(ENV{CMKABE_MINSIZE} ON)\n")
            fwrite(f, "    set(ENV{CMKABE_DBGINFO} OFF)\n")
            fwrite(
                f, 'elseif(CMAKE_BUILD_TYPE_LOWER STREQUAL "relwithdebinfo")\n'
            )
            fwrite(f, "    set(ENV{CMKABE_DEBUG} OFF)\n")
            fwrite(f, "    set(ENV{CMKABE_MINSIZE} OFF)\n")
            fwrite(f, "    set(ENV{CMKABE_DBGINFO} ON)\n")
            ...
            fwrite(
                ...
                'set(ENV{{CMKABE_LINK_DIRS}} "{}")\n'.format(
                    os.path.pathsep.join(
                        self.enum_prefix_subdirs_of("lib", cmake=True)
                    )
                ),
            )
            fwrite(
        ...
        with open(
            os.path.join(self.cmake_target_dir, ".toolchain.cmake"), "wb"
        ) as f:
