...
if(NOT CMAKE_BUILD_TYPE)
    if(CMAKE_INSTALL_CONFIG_NAME)
        set(CMAKE_BUILD_TYPE "${CMAKE_INSTALL_CONFIG_NAME}")
    else()
...
function(cmkabe_add_make_target)
    ...
    # else()
    #     list(APPEND argv "${CMAKE_COMMAND}" "-E" "env" ${CMKABE_ENV_BLOCK})
    # endif()
    list(APPEND argv "make")
    list(APPEND argv ${args_TARGETS})
    list(APPEND argv ${make_options})
    if(args_DEPENDENCIES)
        list(APPEND argv "CMKABE_COMPLETED_PROJECTS=${args_DEPENDENCIES}")
    endif()
...
function(_cmkabe_apply_extra_flags)
    ...
    foreach(dir IN ITEMS "${CARGO_OUT_DIR}" "${CMAKE_BINARY_DIR}")
        if((NOT dir STREQUAL "") AND (NOT "${dir}" IN_LIST paths))
    ...
    if(TARGET_PREFIX_SUBDIRS)
        list(INSERT CMAKE_PREFIX_PATH 0 "${TARGET_PREFIX_SUBDIRS}")
    endif()
    ...
    if(_CMKABE_ALLOW_SET_ROOT_PATH)
        ...
        if(TARGET_PREFIX_SUBDIRS)
            ...
            set(root_path "$(WORKSPACE_DIR)/installed/${TARGET}")
        endif()
        # Lock CMake to only search your third-party SDK directory
        set(CMAKE_FIND_ROOT_PATH "${root_path}" PARENT_SCOPE)
        ...
        # Don't search the tool's system paths
